...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, label_id,aug_mask):
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        aug_mask=[0]+[1]*len(tokens_a)+[0]

        if tokens_b:
            tokens += tokens_b + ["[SEP]"]
            segment_ids += [1] * (len(tokens_b) + 1)
            aug_mask+=[1]*len(tokens_b)+[0]
...
def main():
    ...
    parser.add_argument('--con_model',
                        default=None,
                        type=str,
                        help="Whether to continue training and where the model is")
        ...
        for epoch_i in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            if epoch_i % 1==0:
                ...
                result = {'test_loss': eval_loss,
                          ...
                          'mc': eval_mc}
                logger.info("Test Loss: %s", result)
                f.write("Test Loss: %s\n" % result)
